...
--(SELECT MAX(CTE3.A_PLAN) FROM CTE3 WHERE CTE3.A_FK_ROADMAP = RM.A_ID) AS PIMPACT_AT_INITIATIVE_END_PLAN, 
	--(SELECT MAX(CTE3.A_FORECAST) FROM CTE3 WHERE CTE3.A_FK_ROADMAP = RM.A_ID) AS PIMPACT_AT_INITIATIVE_END_FORECAST,
	--(SELECT MAX(CTE3.A_ACTUAL) FROM CTE3 WHERE CTE3.A_FK_ROADMAP = RM.A_ID) AS PIMPACT_AT_INITIATIVE_END_IMPLEMENTED,
	--(SELECT IIF(MAX(CTE3.A_PLAN)is not NULL, MAX(CTE3.A_ACTUAL)/MAX(CTE3.A_PLAN), CTE3.A_ACTUAL) FROM CTE3 WHERE  CTE3.A_FK_ROADMAP = RM.A_ID) AS PIMPACT_AT_INITIATIVE_END_IMPLEMENTED_PLAN,